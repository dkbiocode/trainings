...
This step creates a session with the name 'assembly'.
...
reads with `-1`, reverse paired-end reads with `-2`, and the output
...
Now that it is running we should detach our screen with `control + a` `d` and wait for a few minutes while it is running. And then attach the screen with
...
SPAdes log can be found here: [WORKSHOP_PATH]/workshops/metagenomics/dc_workshop/results/assembly_JC1A/spades.log
...
[Previous: Trimming and Filtering](03-trimming-filtering.md) | [Next: Metagenome Binning](05-binning.md)
